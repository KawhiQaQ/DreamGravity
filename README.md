...
| 🔮 AI 智能解析 | 象征意义分析，模式识别（压力梦/重复主题/预示性），心理洞察 |
| 🎨 创意生成 | 梦境可视化图片，短篇故事/诗歌改编，专属 IP 角色 |
| 📊 数据可视化 | 时间线/日历视图，元素图谱，梦境星座卡片，统计报告 |
| 📅 周报系统 | 每周梦境总结，周期 IP 角色和图腾，趋势追踪 |
...
| 服务 | 推荐平台 | 配置要点 |
|------|---------|---------|
| 前端 | Vercel | 根目录设置 `frontend`，配置 `VITE_API_URL` |
| 后端 | Railway / Render | 根目录设置 `backend`，配置 `ZHIPU_API_KEY` 和 `FRONTEND_URL` |
