...
> 记录梦境，解析潜意识，构建你的专属梦境宇�?

DreamGravity 是一款基�? AI 的梦境记录与分析应用，帮助你捕捉每一个梦境片段，深入解析梦境背后的象征意义，并将你的梦境世界可视化呈现�?
...
| 🔮 AI 智能解析 | 象征意义分析，模式识别（压力�?/重复主题/预示性），心理洞�? |
| 🎨 创意生成 | 梦境可视化图片，短篇故事/诗歌改编，专�? IP 角色 |
| 📊 数据可视�? | 时间�?/日历视图，元素图谱，梦境星座卡片，统计报�? |
| 📅 周报系统 | 每周梦境总结，周�? IP 角色和图腾，趋势追踪 |
...
# 编辑 .env 文件，填入智�? AI API Key
```

> 💡 获取 API Key：访�? [智谱 AI 开放平台](https://open.bigmodel.cn/)
...
| POST | /api/dreams/:id/generate-creative | 生成创意内容（故�?/诗歌�? |
...
| GET | /api/dreams/:id/followups | 获取梦境的后续关�? |
...
| GET | /api/collective/dreams | 获取公共池梦境列�? |
...
| 服务 | 推荐平台 | 配置要点 |
|------|---------|---------|
| 前端 | Vercel | 根目录设�? `frontend`，配�? `VITE_API_URL` |
| 后端 | Railway / Render | 根目录设�? `backend`，配�? `ZHIPU_API_KEY` �? `FRONTEND_URL` |
